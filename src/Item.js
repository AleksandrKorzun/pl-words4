...
export default class Item extends Phaser.GameObjects.Container {
  ...
  onBaseSelect() {
    ...
    if (this.scene.counter < 4 && !this.isGlow) {
      ...
      if (this.scene.choice.length === 1) {
        this.scene.btn_submit.addBaseInteractive();
      }

      const newTutorialWords = this.scene.tutorialWords.filter(
        (word) => this.img.toUpperCase() !== word
      );
    ...
    if (this.isGlow && this.scene.counter > 0) {
      ...
      if (!choice.length) {
        console.log("dis");
        this.scene.btn_submit.removeInteractive();
      }
      this.scene.choice = choice;
      this.isGlow = false;
    }
    if (this.scene.tutorialWords[0]) {
      this.scene.items.showHand(this.scene.tutorialWords[0], 500);
    }
  ...
  onClick() {
    if (!this.scene.isStartGame) {
      setTimeout(() => this.scene.game.network.openStore(), 20000);
  ...
  onItemsClick() {
    if (
      this.scene.counter === 4 &&
      this.scene.isTutorial &&
      !this.scene.tutorialWords.length
    ) {
